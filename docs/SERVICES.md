...
- `debugLog` compiles out in release on purpose, so a released build leaks nothing. The two stream paths where work is actually lost — a connection error and a dropped event — use `faultLog` instead, which reaches the system log in every build and carries no wallet data. Add a release-safe line only where the app drops something a field report would otherwise never show.
