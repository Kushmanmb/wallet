...
- **R20** **M** Chain settings and nodes — `ChainSettingsSceneViewModel`, `ChainNodeViewModel` and `ServiceStatusItemViewModel` on iOS against the Android networks screens. `GemNodeSelection` and `GemServiceEndpoint` cross, but the section titles, the node subtitle and the explorer row are each app's. The selected explorer is also the one preference no screen can observe: `GemAssetDetailsService.details` reads it inside Core, so `AssetSceneViewModel.details` cannot be derived through an `Equatable` input until the chosen explorer is observable at the preferences owner. Make it observable here, then derive that screen's details the way `WalletSceneViewModel.homeState` does.
