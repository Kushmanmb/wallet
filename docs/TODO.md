...
- **PERF14** **S** Give swap price subscriptions one trigger — [`SwapSceneViewModel.setLoadTrigger`](../ios/Features/Swap/Sources/ViewModels/SwapSceneViewModel.swift) launches an unstructured `addPrices` task for each valid amount/slippage change, although only the asset IDs matter. Move registration into the model's existing asset-ID lifecycle action alongside balance loading, keeping the work independently awaited so subscription latency does not delay balances. Remove the task from quote-trigger construction. Core already deduplicates subscribed IDs; retain its reconnect behavior rather than adding an app cache. Verify initial pair, pair changes/reversal, and screen reentry register prices, while typing, Max, and slippage edits create no subscription work. Follow Android's asset-selection subscription boundary; preserve quote debounce, refresh, and selection behavior.
...
- **PERF16** **S** Publish images only for the current request — [`CachedAsyncImage.load`](../ios/Packages/Components/Sources/CachedAsyncImage.swift) assigns success/failure after awaiting a shared load without checking cancellation. A canceled task can still receive that shared result. Check task cancellation before publishing either outcome; if request identity is also tracked, include URL, size, and scale. Keep shared load reuse so dismissing one consumer does not cancel another consumer's image. Verify delayed A followed by B, a canceled failure after B succeeds, and two consumers of one load. This gives image state one guarded publication boundary and prevents obsolete results from replacing current icons. It can land independently of PERF15.

### Implementation candidates

- **PERF2** **M** iOS derived wallet and asset state — [`WalletSceneViewModel.homeState`](../ios/Features/WalletTab/Sources/ViewModels/WalletSceneViewModel.swift) and [`AssetSceneViewModel.details`](../ios/Features/Assets/Sources/ViewModels/AssetSceneViewModel.swift) recompute Core answers and display projections on repeated reads. Follow [`DerivedValue`](../ios/Packages/PrimitivesComponents/Sources/Types/DerivedValue.swift) and [`FiatSceneViewModel`](../ios/Features/FiatConnect/Sources/ViewModels/FiatSceneViewModel.swift): derive through named `Equatable` inputs in the existing model, and make dependent getters reuse that answer. First account for hidden service inputs: selected wallet/collateral preference in home state and explorer preference in asset details. Expose missing invalidation through the existing service/observation owner before caching those calls; a cache keyed only by visible balances or asset ID is incorrect. Verify unchanged inputs reuse the result and each data/preference change refreshes it. Keep store-backed work out of render-time evaluation and preserve the existing Core records; R24 remains a separate row migration.
- **PERF3** **M** Android wallet row reuse — [`GetActiveAssetsInfoImpl`](../android/data/coordinators/src/main/kotlin/com/gemwallet/android/data/coordinators/asset/GetActiveAssetsInfoImpl.kt) formats the entire list before `distinctUntilChanged`. Reuse unchanged row inputs within the existing coordinator, following the collection-scoped cache in [`TransactionRows`](../android/data/coordinators/src/main/kotlin/com/gemwallet/android/data/coordinators/transaction/GetTransactionsImpl.kt). Retain only the current list; include wallet context, row naming, privacy, currency, and locale in invalidation. Keep formatting on the worker dispatcher and preserve Core's value/style decisions. Extend [`GetActiveAssetsInfoImplTest`](../android/data/coordinators/src/test/kotlin/com/gemwallet/android/data/coordinators/asset/GetActiveAssetsInfoImplTest.kt): one changed price rebuilds only its row, removed rows leave the cache, and wallet/privacy/currency changes cannot reuse stale output.
- **PERF4** **M** Android confirmation preparation — [`ConfirmViewModel`](../android/features/confirm/viewmodels/src/main/kotlin/com/gemwallet/android/features/confirm/viewmodels/ConfirmViewModel.kt) decodes the route and builds [`Simulation`](../android/features/confirm/viewmodels/src/main/kotlin/com/gemwallet/android/features/confirm/viewmodels/Simulation.kt) on Main. Put decoding and simulation mapping on worker execution at their existing boundaries, including synchronous Core calls that construct explorer links. Capture the matching request, confirmation, simulation, and load before dispatch; publish only if that context is still current. Retain the existing `GemConfirmation`, wire format, validation, authentication, and send path. Test a large payload, malformed input, rapid request/fee replacement, cancellation, and delayed old results; no previous request may replace current fields or enable the button. Malformed input must not become a success fallback.
- **PERF5** **M** Android transaction date sections — [`dateGroupedList`](../android/ui/src/main/kotlin/com/gemwallet/android/ui/components/list_item/DateGroupedList.kt) groups and converts timestamps inside lazy-list composition. Prepare sections in the existing transaction presentation flow, and have the list render prepared sections with stable transaction IDs. Keep device-time-zone grouping app-side; reuse Core's day boundaries once per section build per [Keep the crossings few](ARCHITECTURE.md#keep-the-crossings-few). Reuse grouping when only prices/status change, while binding the latest row values. Invalidate on insertion/removal, timestamp/order, time zone, local day, and locale where labels depend on it. Test midnight, DST, equal timestamps, new transactions while scrolled, and unchanged row identity. Preserve the current query limit and other shared-list consumers; pagination and a shared date service are outside this item.
- **PERF6** **M** Core asset refresh overlap — [`GemAssetDetailsService.refresh`](../core/gemstone/src/services/assets/details.rs) serially awaits subscription, metadata/associations, balances, and transactions. Preserve subscription and required asset setup, then run balance refresh and transaction sync concurrently through the same service. Check their store reads/writes first and retain any actual dependency. Capture the wallet once, await both branches, and collect failures in the existing step order; a fast failure must not cancel the other branch. Extend the owner's injected-client/store tests to hold either branch pending, prove overlap, and verify the same final records, sync cursor, and per-step errors. Keep signatures, app entry points, refresh timers, and Core ownership unchanged; exercise asset refresh on both apps.
