...
- **F7** **S** `FiatSceneViewModel` still builds a `CurrencyFormatter` for the currency symbol and the `GemFiatAmountCheck` minimum and maximum, and a `ValueFormatter` for the balance. The rate and both quote amounts now cross as `GemFormattedNumber`; what is left is carrying the amount-check bounds the same way, and deciding whether the currency symbol is a Core answer or locale data.
