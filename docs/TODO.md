...
- **O12** **M** [`GemWalletService.swift`](../ios/Packages/GemstonePrimitives/Sources/Services/GemWalletService.swift) is sixty lines of `wallet.id.id` and `.toPrimitives()` — a bridge with no decisions in it, which [§ 6](ARCHITECTURE.md#6-where-derived-domain-answers-live) allows for persisted types. It is that long because `WalletId` is a struct in the apps and a bare `String` across the FFI, so every call unwraps it by hand, on both platforms. Make `WalletId` cross as the record the apps already hold, the way `AssetId` does, and most of the file and its Kotlin counterpart go with it. Check first whether anything persists a wallet id as a raw string.
