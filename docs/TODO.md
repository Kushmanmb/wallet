...
- **X45** **M** 76 production `unwrap`/`expect` sites remain in `core/`, recounted on 2026-09-16. The 255 from the first count included every `#[cfg(all(test, feature = "chain_integration_tests"))]` module, every `#[cfg(test)] impl`, every bare `#[test]` function and every `src/testkit/` directory, which is why it named `gem_stellar/provider/transactions.rs`, `gem_bitcoin/provider/transactions.rs` and `gem_solana/provider/transactions.rs` as the leaders — all three are integration-test modules with nothing to fix. What is left falls in three groups. Boot wiring that is meant to stop the process — `apps/daemon/src/main.rs` (6), `apps/daemon/src/setup/production.rs`, `apps/api/src/main.rs`, `settings/src/lib.rs`, `search_index/src/lib.rs`, `storage/database/migrations.rs`, `localizer/src/lib.rs` and the six reqwest-builder sites — is the group to leave alone unless the daemon should degrade instead of refusing to start. Values that are infallible by construction — `Chain::from_str(self.as_ref())` on the chain enums, a const base58 or BOC or URL parsed once, an HMAC that accepts any key length, and every `X::from_chain(chain).unwrap()` inside a matching `chain_type()` arm in `settings_chain/src/lib.rs`, `gemstone/gateway/chain_factory.rs` and `gemstone/signer/chain.rs` — needs the type to carry the guarantee, not a different call. What is genuinely left to fix is the mutex locks in `apps/daemon/src/metrics/` (7) and `apps/dynode/`, the swapper provider constructors that `expect` an endpoint, `Explorer::new` taking a chain string it cannot reject, and `AssetId::from(&str)` and `QuoteAsset::asset_id` panicking on a malformed id.
