...
- **X20** **M** Catch-all branches defeat the exhaustiveness that makes a Core enum safe to extend: Android has 14 `when` blocks over a `Gem*` enum ending in `else ->`, iOS has 3 `switch` blocks ending in `default:`. Two examples show the cost — `GemSimulationWarningKind` in [`ui/.../GemstoneText.kt`](../android/ui/src/main/kotlin/com/gemwallet/android/ui/localization/GemstoneText.kt) sends any new warning kind to a generic description, and `GemConfirmException` in [`ConfirmErrorInfo.kt`](../android/features/confirm/presents/src/main/kotlin/com/gemwallet/android/features/confirm/presents/components/ConfirmErrorInfo.kt) returns no asset for any error variant added later. List the variants instead, so adding one to Core fails the build on the app that has to decide.

- **X19** **S** Dead code, most of it orphaned by migrations that already landed: Android [`Precision.kt`](../android/gemcore/src/main/kotlin/com/gemwallet/android/model/Precision.kt) still declares `twoPlaces`, `fourSignificant` and `ABBREVIATION_THRESHOLD` after the value ladder moved to Core, and iOS has six unread `Localized` keys, five unused `AuthenticationPolicy` cases, nine of the fifteen `Locale` identifiers in `Locale+Primitives.swift` (`IT`, `PT_BR`, `PT_PT`, `EN_CH`, `DE_CH`, `AR_SA` and the three `ZH_*`), which are test locales the tests stopped using, `String.numberOfOccurrencesOf`, `DevicePlatform.deviceInfo`, `WalletConnectorInteractor.signTransaction` and `DefaultCryptoProvider.recoverPubKey`. A sweep that re-finds these must skip the false-positive classes: `Gem*Store` foreign-trait implementations on both apps, Hilt `@Provides`, Room `TypeConverters`, `@Preview` composables and framework overrides are all called by something a token search cannot see.
