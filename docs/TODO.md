...
- **X45** **M** 106 production `unwrap`/`expect` sites remain in `core/`, recounted on 2026-09-16. The 255 from the first count included every `#[cfg(all(test, feature = "chain_integration_tests"))]` module and every `src/testkit/` directory, which is why it named `gem_stellar/provider/transactions.rs`, `gem_bitcoin/provider/transactions.rs` and `gem_solana/provider/transactions.rs` as the leaders — all three are integration-test modules with nothing to fix. What is left is led by `apps/daemon/src/main.rs` (6), `settings_chain/src/lib.rs` (4), `gem_aptos/provider/payload_builder.rs` (3), `primitives/secure_preferences.rs` (3), `gemstone/gateway/chain_factory.rs` (3) and `apps/daemon/src/metrics/parser.rs` (3). Take one file at a time and either make the value infallible by construction — the way the explorers build their metadata directly instead of looking a key back up — or return the error.
