...
- **O11** **S** `AmountStakeProvider.buildTransfer` and `AmountEarnProvider.buildTransfer` read `assetInfo.value` and `error("assetInfo not loaded")` when it is still the `stateIn` seed. Nothing makes the asset arrive before a submit, so `AmountStakeProviderTest.delegate builds DelegateParams` fails intermittently and a fast tap could throw in the app. Await the first non-null value instead of reading the seed, and check whether iOS's provider has the same gap.
...
- **F14** **M** Chart numbers — `GemChartHeader` (value, secondary value, change percentage), `GemCandleTooltip` (open, high, low, close, change, volume) and `GemPerpetualChartLine` (price) all cross as bare `f64` and each app picks the style. The header needs two additions to the number contract first, because its `PriceChange` type renders a *signed* currency and a percentage *in parentheses*, and both apps decide each separately: give `GemFormattedNumber` a sign rule and `GemNumberUnit::Percent` a parenthesised form, then thread the display currency into `chart_header`. The tooltip and the chart line are plain numbers and need neither.
- **F16** **S** Closed on 2026-09-14: none of the three is rendered. `GemPriceUpdate` is written to the price table by both stores, `GemAssetDetailsInput.price` is an input Core consumes and no app reads back, and `GemPriceAlertSession`'s `input` and `current_price` feed Core's validation — the screens format the price they show from their own asset info, never from the session. What is worth deciding: both apps compose that "Current price X" line themselves and disagree on emphasis, iOS plain and Android bolding the value. One line, one screen, either is defensible — pick one and let the row say it.
- **F18** **S** Closed on 2026-09-14: none of the three is a rendered number. `GemBalanceValue.amount` is written straight to the balance table by both stores, `GemNftRow.count` is widened to an `Int` and handed to a grid poster that decides how to show it, and `invite_reward_points` is bolded as a bare integer inside the same shared sentence on both apps — no separator, precision or style is chosen anywhere. A future bare-number sweep should keep splitting rendered numbers from stored and layout ones before listing them.
