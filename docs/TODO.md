...
- **F7** **S** `FiatSceneViewModel` keeps a `CurrencyFormatter` only for the currency symbol and a `ValueFormatter` for the balance, which is a `BigInt` and cannot cross as an `f64`. Every number the session hands it now crosses as `GemFormattedNumber`. What is left is deciding whether the currency symbol is a Core answer or locale data.
