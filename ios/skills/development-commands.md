...
just check-test Primitives  # run ONE package's tests with SwiftPM (~2s), Gemstone-free packages only
...
|---|---|---|
| `just check <Package>` | 1-2s | does this package still compile — the default while editing |
| `just check-test <Package>` | 2s | that package's tests, when it does not depend on Gemstone |
