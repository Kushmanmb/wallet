...
use prices::{PriceAssetsProvider, PriceProvider, PriceProviders};
...
pub async fn jobs(ctx: WorkerContext, shutdown_rx: ShutdownReceiver) -> Result<Vec<JobHandle>, Box<dyn Error + Send + Sync>> {
    ...
    let assets_providers: AssetsProviders = Arc::new(services.price_providers(enabled_providers.iter().copied()));
            ...
            move |_| {
                ...
                async move {
                    let observed_config = ObservedPricesConfig {
                        max_assets: config.get_usize(ConfigKey::PriceObservedMaxAssets).await?,
                        min_observers: config.get_usize(ConfigKey::PriceObservedMinObservers).await?,
...
{
    let variant = JobVariant::labeled(job, kind).with_param_duration(config, &interval).await?;
        ...
        Box::pin(async move {
            match action {
                ...
                ChartsAction::Delete(tf) => {
                    let retention = config.get_duration(charts_retention_key(tf)).await?;
