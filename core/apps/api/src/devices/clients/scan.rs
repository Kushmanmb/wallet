...
use primitives::{AssetId, ChainAddress, ScanProvider, ScanSource, ScanTransaction, ScanTransactionPayload, ScanType, TransactionType, asset_score::AssetRank};
use reqwest::Url;
...
impl ScanDetection {
    fn new(scan_type: ScanType, finding: ScanFinding, is_enforced: bool, source: impl Into<String>) -> Self {
        Self {
            scan_type,
            finding,
            is_enforced,
...
impl ScanClient {
    ...
    pub async fn get_scan_transaction(&self, payload: ScanTransactionPayload) -> Result<ScanTransaction, Box<dyn Error + Send + Sync>> {
        let enforced = self.get_enforced_types()?;
        let website_host = payload.website.as_deref().and_then(Self::website_host);
        let local = self.get_scan_transaction_local(&payload, &enforced)?;
        let cached = self.get_cached_detections(&payload, website_host.as_deref(), &enforced, local.is_target_verified)?;
        ...
        let is_scanned = |scan_type: ScanType| !cached.iter().any(|detection| detection.scan_type == scan_type);
        ...
        for (scan_type, checks) in scans {
            ...
            let is_enforced = enforced.contains(&scan_type);
            if is_enforced {
                new_detections.push(NewScanDetectionRow::new(scan_type, chain, target, *provider, check.reason.clone()));
            }
            detections.push(ScanDetection::new(scan_type, finding, is_enforced, ScanDetection::provider_source(*provider, check.reason.as_deref())));
        }
        self.database.scan_detections()?.add_scan_detections(new_detections)?;

        let completed_scans = scans
            .iter()
            .filter(|(scan_type, _)| enforced.contains(scan_type))
    ...
    fn get_enforced_types(&self) -> Result<HashSet<ScanType>, Box<dyn Error + Send + Sync>> {
    ...
    fn get_scan_transaction_local(&self, payload: &ScanTransactionPayload, enforced: &HashSet<ScanType>) -> Result<LocalScan, Box<dyn Error + Send + Sync>> {
        ...
        let mut detections = addresses
            ...
            .map(|address| ScanDetection::new(ScanType::Address, ScanFinding::Address(ChainAddress::new(address.chain.0, address.address.clone())), true, "manual"))
            .collect::<Vec<_>>();
        let is_asset_enforced = enforced.contains(&ScanType::Asset);
    ...
    fn get_cached_detections(&self, payload: &ScanTransactionPayload, website_host: Option<&str>, enforced: &HashSet<ScanType>, is_target_verified: bool) -> Result<Vec<ScanDetection>, Box<dyn Error + Send + Sync>> {
        ...
        Ok(targets
            ...
            .filter_map(|(scan_type, target_chain, target)| {
                ...
                Some(ScanDetection::new(scan_type, finding, enforced.contains(&scan_type), source))
            })
            .collect())
    }
...
mod tests {
    ...
    fn test_scan_transaction_excludes_dry_run_detections() {
        ...
        let detections = vec![
            ScanDetection::new(ScanType::Address, ScanFinding::Address(address.clone()), true, "manual"),
            ScanDetection::new(ScanType::AddressPoisoning, ScanFinding::Address(address.clone()), true, "hashdit: is_poisoning"),
            ScanDetection::new(ScanType::Website, ScanFinding::Website("https://example.com".to_string()), false, "hashdit: phishing"),
            ScanDetection::new(ScanType::Asset, ScanFinding::Asset(AssetId::from_token(Chain::SmartChain, "0x456")), false, "smartchain_0x456"),
        ];
    ...
    fn test_scan_transaction_dry_run_only_is_not_malicious() {
        let detections = vec![ScanDetection::new(ScanType::Website, ScanFinding::Website("https://example.com".to_string()), false, "hashdit: phishing")];
