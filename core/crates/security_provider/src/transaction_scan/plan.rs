...
use super::subject::{ScanSubject, scan_subjects, website_host};
...
pub fn plan_transaction_scan(input: &TransactionScanInput) -> ScanPlan {
    ...
    let cached = cached_detections(input, &scan_subjects(payload), is_target_verified);
    ...
    let targets = provider_targets(payload).filter(|_| !is_resolved).map(|targets| targets.retain(|scan_type| !is_skipped(scan_type)));
