...
impl WalletsRepository for DatabaseClient {
    fn get_wallet(&mut self, identifier: &str) -> Result<WalletRow, DatabaseError> {
        wallet_row(self, identifier).or_not_found(identifier.to_string())
    }
    ...
    fn get_wallets(&mut self, identifiers: Vec<String>) -> Result<Vec<WalletRow>, DatabaseError> {
        Ok(wallets::table.filter(wallets::identifier.eq_any(identifiers)).select(WalletRow::as_select()).load(&mut self.connection)?)
    }

    fn create_wallets(&mut self, new_wallets: Vec<NewWalletRow>) -> Result<usize, DatabaseError> {
        Ok(diesel::insert_into(wallets::table).values(&new_wallets).on_conflict(wallets::identifier).do_nothing().execute(&mut self.connection)?)
    }

    fn get_or_create_wallet(&mut self, wallet: NewWalletRow) -> Result<WalletRow, DatabaseError> {
        match wallet_row(self, &wallet.identifier) {
            Ok(existing) => Ok(existing),
            Err(diesel::result::Error::NotFound) => Ok(diesel::insert_into(wallets::table).values(&wallet).returning(WalletRow::as_returning()).get_result(&mut self.connection)?),
            Err(error) => Err(error.into()),
        }
    }

    fn get_subscriptions(&mut self, device_id: i32) -> Result<Vec<(WalletRow, WalletSubscriptionRow, WalletAddressRow)>, DatabaseError> {
        Ok(wallets_subscriptions::table
    ...
    fn add_subscriptions(&mut self, device_id: i32, subscriptions: Vec<(i32, Chain, String)>) -> Result<usize, DatabaseError> {
        ...
        let existing_rows = self.get_addresses(all_addresses.clone())?;
        ...
        let new_rows = if missing_addresses.is_empty() {
            ...
            self.get_addresses(missing_strs)?
        };
    ...
    fn delete_subscriptions(&mut self, device_id: i32, subscriptions: Vec<(i32, Chain, String)>) -> Result<usize, DatabaseError> {
        ...
        let address_rows = self.get_addresses(all_addresses)?;
    ...
    fn add_subscriptions_exclude_addresses(&mut self, values: Vec<SubscriptionAddressExcludeRow>) -> Result<usize, DatabaseError> {
        Ok(diesel::insert_into(subscriptions_addresses_exclude::table).values(values).on_conflict_do_nothing().execute(&mut self.connection)?)
    }

    fn get_subscriptions_exclude_addresses(&mut self, addresses: Vec<String>) -> Result<Vec<String>, DatabaseError> {
        Ok(subscriptions_addresses_exclude::table
    ...
    fn get_addresses(&mut self, addresses: Vec<String>) -> Result<Vec<WalletAddressRow>, DatabaseError> {
        Ok(wallets_addresses::table.filter(wallets_addresses::address.eq_any(addresses)).select(WalletAddressRow::as_select()).load(&mut self.connection)?)
    }
}
